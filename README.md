...
In version `v1.25.0`, we added a new option `--template/-t` for the `sqlx` mode, which is used to add uniform 
template content to all SQL query statements. This is experimental and requires more testing.
...
When the `api/ignore-status` feature is enabled, the code generated by `defc` will no longer include checks for values of `http.Response.StatusCode`. Even if a service returns a status code other than `http.StatusOK`, an additional `error` will not be returned. The caller needs to handle the status codes themselves and provide and return corresponding errors based on the status codes.
