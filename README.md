...
- `sqlx/strict-merge`: **(v1.45.0)** Error on duplicate bind keys
  contributed by different arguments. The generated call site
  invokes `runtime.MergeNamedArgsStrict` and propagates the error
  alongside the existing `sqlx.Named` error path. See the
  `🔒 Security — MergeNamedArgs collision` entry in `CHANGELOG.md`
  for rollout details.
