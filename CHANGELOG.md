...
- **`runtime.MergeNamedArgs` silently overwrote bind keys on
  ...
  **authorization-scope bypass in the generated SQL**.
  ...
  `MergeNamedArgs` is unchanged and is preserved bit-for-bit from
  prior releases: on collision, last-writer-wins under Go's
  randomised map iteration. New opt-in API gives callers strict
  detection:
  ...
  New feature flag (generator-level, opt-in for v1.45.0):

  - `sqlx/strict-merge` — emit the two-return form in
    `gen/template/sqlx.tmpl` and a matching error-propagation block
    modelled on the existing `sqlx.Named` handling. In `sqlx/nort`
    mode, the inlined `__<Ident>MergeNamedArgs` function changes
    signature to `(map[string]any, error)` and performs the same
    collision detection without importing `__rt`.

  Rollout plan:

  - **v1.45.0 (this release)** — `MergeNamedArgs` unchanged;
    `MergeNamedArgsStrict` + `sqlx/strict-merge` available as opt-in.
  ...
  `runtime/merge_fuzz_test.go` provides a property-based fuzz
  target asserting (1) strict never panics, (2) every strict error
  wraps `ErrNamedArgsCollision`, and (3) legacy and strict agree on
  the set of emitted keys when strict succeeds.
  `gen/strict_emission_test.go` covers both runtime-backed and
  nort-inlined strict emission.
...
- **Generated `sqlx`- and `api`-mode code interpolated schema
  ...
  byte-identical when the flags are absent.
