...
// runTestRetain is a sibling of runTest that returns the generated bytes
// without deleting the output file. The caller is responsible for cleanup
// (typically via t.Cleanup(func() { os.Remove(path) })).
func runTestRetain(path string, builder *CliBuilder) ([]byte, error) {
	var bf bytes.Buffer
	if err := builder.Build(&bf); err != nil {
		return nil, err
	}
